...
def disentangle_beta_loss(recon_x,x,mu,logvar,beta,global_iter,gamma=1000,C_max=25,C_stop_iter=25):
    C_max = Variable(torch.FloatTensor([C_max]).cuda())
    recon_loss = nn.CrossEntropyLoss()(recon_x,x)  # gaussian or bernoulli
    KLD = -0.5 * torch.sum(1+logvar-mu.pow(2)-logvar.exp())
    C = torch.clamp(C_max/C_stop_iter*global_iter,0,C_max.data[0])
    beta_vae_loss = recon_loss + gamma * (KLD-C).abs()
...
def custom_loss(model_output,y):
    ...
    BCE = F.binary_cross_entropy_with_logits(recon_x.view(-1,784),x.view(-1,784),reduction='sum')
    KLD = -0.5 * torch.sum(1+logvar-mu.pow(2)-logvar.exp())
    ENTROPY = F.cross_entropy(yp,y,reduction='sum')
    return ENTROPY + BCE + beta*KLD 
...
beta_list = np.linspace(3,5,5).tolist()
b = 0
epochs = 20
file_path='./FGSM_result/beta_'
attack = fgsm
...
for beta in beta_list:
    ...
    opt = optim.Adam(model_concetenate.parameters(),lr=1e-4)
